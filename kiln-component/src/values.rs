...
type KilnFormatValType = FormatValType;
type KilnTypesValType = TypesValType<ComponentProvider>;
...
/// Convert from CanonicalValType to kiln_format::component::ValType
pub fn convert_common_to_format_valtype(common_type: &CanonicalValType) -> KilnFormatValType {
    match common_type {
        CanonicalValType::Bool => KilnFormatValType::Bool,
        CanonicalValType::S8 => KilnFormatValType::S8,
        CanonicalValType::U8 => KilnFormatValType::U8,
        CanonicalValType::S16 => KilnFormatValType::S16,
        CanonicalValType::U16 => KilnFormatValType::U16,
        CanonicalValType::S32 => KilnFormatValType::S32,
        CanonicalValType::U32 => KilnFormatValType::U32,
        CanonicalValType::S64 => KilnFormatValType::S64,
        CanonicalValType::U64 => KilnFormatValType::U64,
        CanonicalValType::F32 => KilnFormatValType::F32,
        CanonicalValType::F64 => KilnFormatValType::F64,
        CanonicalValType::Char => KilnFormatValType::Char,
        CanonicalValType::String => KilnFormatValType::String,
        CanonicalValType::Ref(idx) => KilnFormatValType::Ref(*idx),
        CanonicalValType::Record(_fields) => {
            // Record uses ValTypeRef and WasmName which need resolution
            // For now, return empty record
            KilnFormatValType::Record(Vec::new())
        },
        CanonicalValType::Variant(_cases) => {
            // Variant uses ValTypeRef and WasmName which need resolution
            // For now, return empty variant
            KilnFormatValType::Variant(Vec::new())
        },
        CanonicalValType::List(_elem_type_ref) => {
            // List uses ValTypeRef which needs resolution via type table
            // For now, return a placeholder
            KilnFormatValType::List(Box::new(KilnFormatValType::Void))
        },
        CanonicalValType::Tuple(_types) => {
            // Tuple uses ValTypeRef which needs resolution via type table
            // For now, return empty tuple
            KilnFormatValType::Tuple(Vec::new())
        },
        CanonicalValType::Flags(names) => {
            ...
            let string_names: Vec<String> = names
                ...
                .collect();
            KilnFormatValType::Flags(string_names)
        },
        CanonicalValType::Enum(variants) => {
            ...
            let string_variants: Vec<String> = variants
                ...
                .collect();
            KilnFormatValType::Enum(string_variants)
        },
        CanonicalValType::Option(_inner_type) => {
            // Option requires ValTypeRef resolution which needs a type table
            // For now, return a placeholder
            KilnFormatValType::Option(Box::new(KilnFormatValType::Void))
        },
        CanonicalValType::Own(idx) => KilnFormatValType::Own(*idx),
        CanonicalValType::Borrow(idx) => KilnFormatValType::Borrow(*idx),
        CanonicalValType::FixedList(_elem_type_ref, size) => {
            // FixedList uses ValTypeRef which needs resolution via type table
            KilnFormatValType::FixedList(Box::new(KilnFormatValType::Void), *size)
        },
        CanonicalValType::Void => {
            // Void doesn't have a direct mapping, convert to a unit tuple
            KilnFormatValType::Tuple(Vec::new())
        },
        CanonicalValType::ErrorContext => KilnFormatValType::ErrorContext,
        CanonicalValType::Result { ok: _, err: _ } => {
            // For KilnFormatValType, we create a Result with a generic type placeholder
            // Since KilnFormatValType::Result requires a concrete type, we'll use Void
            KilnFormatValType::Result(Box::new(KilnFormatValType::Void))
        },
    }
}

/// Convert from kiln_format::component::ValType to CanonicalValType
///
...
pub fn convert_format_to_common_valtype(format_type: &KilnFormatValType) -> CanonicalValType {
    match format_type {
        KilnFormatValType::Bool => CanonicalValType::Bool,
        KilnFormatValType::S8 => CanonicalValType::S8,
        KilnFormatValType::U8 => CanonicalValType::U8,
        KilnFormatValType::S16 => CanonicalValType::S16,
        KilnFormatValType::U16 => CanonicalValType::U16,
        KilnFormatValType::S32 => CanonicalValType::S32,
        KilnFormatValType::U32 => CanonicalValType::U32,
        KilnFormatValType::S64 => CanonicalValType::S64,
        KilnFormatValType::U64 => CanonicalValType::U64,
        KilnFormatValType::F32 => CanonicalValType::F32,
        KilnFormatValType::F64 => CanonicalValType::F64,
        KilnFormatValType::Char => CanonicalValType::Char,
        KilnFormatValType::String => CanonicalValType::String,
        KilnFormatValType::Ref(idx) => CanonicalValType::Ref(*idx),
        KilnFormatValType::Record(_fields) => {
            // Cannot convert without memory provider for BoundedVec and WasmName
            CanonicalValType::Void
        },
        KilnFormatValType::Variant(_cases) => {
            // Cannot convert without memory provider for BoundedVec and WasmName
            CanonicalValType::Void
        },
        KilnFormatValType::List(_elem_type) => {
            // Cannot convert without type table for ValTypeRef
            CanonicalValType::Void
        },
        KilnFormatValType::Tuple(_types) => {
            // Cannot convert without type table for ValTypeRef
            CanonicalValType::Void
        },
        KilnFormatValType::Flags(_names) => {
            // Cannot convert without memory provider for BoundedVec and WasmName
            CanonicalValType::Void
        },
        KilnFormatValType::Enum(_variants) => {
            // Cannot convert without memory provider for BoundedVec and WasmName
            CanonicalValType::Void
        },
        KilnFormatValType::Option(_inner_type) => {
            // Cannot convert without type table for ValTypeRef
            CanonicalValType::Void
        },
        KilnFormatValType::Result(_result_type) => {
            // Cannot convert without type table for ValTypeRef
        ...
        KilnFormatValType::Own(idx) => CanonicalValType::Own(*idx),
        KilnFormatValType::Borrow(idx) => CanonicalValType::Borrow(*idx),
        KilnFormatValType::FixedList(_elem_type, size) => {
            // Cannot convert without type table for ValTypeRef
            CanonicalValType::FixedList(ValTypeRef(0), *size)
        },
        KilnFormatValType::ErrorContext => CanonicalValType::ErrorContext,
...
pub fn serialize_component_value_with_stream<'a, P: kiln_foundation::MemoryProvider>(
...
pub fn deserialize_component_value(
    data: &[u8],
    format_type: &KilnFormatValType,
) -> Result<ComponentComponentValue> {
    let mut offset = 0;
    match format_type {
        KilnFormatValType::Bool => {
            if offset >= data.len() {
        ...
        KilnFormatValType::F32 => {
            ...
            let value = kiln_foundation::float_repr::FloatBits32::from_bits(bits);
            Ok(ComponentComponentValue::F32(value))
        },
        KilnFormatValType::F64 => {
            if offset + 8 > data.len() {
            ...
            let value = kiln_foundation::float_repr::FloatBits64::from_bits(bits);
            Ok(ComponentComponentValue::F64(value))
        },
        KilnFormatValType::Char => {
            if offset >= data.len() {
        ...
        KilnFormatValType::FixedList(_elem_type, _size) => {
        ...
        KilnFormatValType::ErrorContext => {
...
pub fn deserialize_component_value_with_stream<'a, P: kiln_foundation::MemoryProvider>(
    ...
    match format_type {
        ...
        KilnFormatValType::F32 => {
            ...
            Ok(ComponentComponentValue::F32(
                kiln_foundation::float_repr::FloatBits32::from_f32(value),
            ))
        },
        KilnFormatValType::F64 => {
            let value = reader.read_f64_le()?;
            Ok(ComponentComponentValue::F64(
                kiln_foundation::float_repr::FloatBits64::from_f64(value),
            ))
        },
        KilnFormatValType::Char => {
...
pub fn serialize_component_values_with_stream<'a, P: kiln_foundation::MemoryProvider>(
...
pub fn deserialize_component_values_with_stream<'a, P: kiln_foundation::MemoryProvider>(
    reader: &mut ReadStream<'a>,
    types: &[KilnFormatValType],
    provider: &P,
) -> Result<Vec<ComponentComponentValue>> {
...
pub fn core_to_component_value(
    value: &Value,
    ty: &KilnFormatValType,
) -> kiln_error::Result<ComponentComponentValue> {
...
pub fn component_to_core_value(value: &ComponentComponentValue) -> kiln_error::Result<Value> {
...
pub fn size_in_bytes(ty: &KilnFormatValType) -> usize {
    match ty {
        KilnFormatValType::Bool => 1,
        KilnFormatValType::S8 => 1,
        KilnFormatValType::U8 => 1,
        KilnFormatValType::S16 => 2,
        KilnFormatValType::U16 => 2,
        KilnFormatValType::S32 => 4,
        KilnFormatValType::U32 => 4,
        KilnFormatValType::S64 => 8,
        KilnFormatValType::U64 => 8,
        KilnFormatValType::F32 => 4,
        KilnFormatValType::F64 => 8,
        KilnFormatValType::Char => 4,
        KilnFormatValType::String => 8, // Pointer size
        KilnFormatValType::Ref(_) => 4,
        KilnFormatValType::Record(_) => 8,
        KilnFormatValType::Variant(_) => 8,
        KilnFormatValType::List(_) => 8,
        KilnFormatValType::Tuple(_) => 8,
        KilnFormatValType::Flags(_) => 8,
        KilnFormatValType::Enum(_) => 4,
        KilnFormatValType::Option(_) => 8,
        KilnFormatValType::Result(_) => 8,
        KilnFormatValType::Own(_) => 4,
        KilnFormatValType::Borrow(_) => 4,
        KilnFormatValType::FixedList(_, _) => 8,
        KilnFormatValType::ErrorContext => 4,
