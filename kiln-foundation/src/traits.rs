...
use crate::{
    ...
    VerificationLevel,
}; // Keep kiln_error::Result, Added RootMemoryProvider etc. // Added KilnError,
   // ErrorCategory, codes
...
pub trait ToFormat<T>: Sized {
    ...
    fn to_format(&self) -> kiln_error::Result<T>;
}
...
/// Trait for types that can be converted to Kiln Value representation
pub trait ToKilnValue {
    /// Converts self to the target Kiln Value type.
    ///
    /// # Errors
    ///
    /// Returns an error if the conversion fails.
    fn to_kiln_value(&self) -> kiln_error::Result<crate::types::ValueType>;
}
...
pub trait BytesWriter {
    ...
    fn write_byte(&mut self, byte: u8) -> kiln_error::Result<()>;
    ...
    fn write_all(&mut self, bytes: &[u8]) -> kiln_error::Result<()>;
}
...
impl FromBytes for bool {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        match byte {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(KilnError::runtime_execution_error(
...
macro_rules! impl_little_endian_for_primitive {
    ($($T:ty, $size:expr);*) => {
        $(impl LittleEndian for $T {
            fn from_le_bytes(bytes: &[u8]) -> kiln_error::Result<Self> {
                if bytes.len() < $size {
                    return Err(kiln_error::Error::new(kiln_error::ErrorCategory::Memory,
                        kiln_error::codes::BUFFER_TOO_SMALL,
...
// fn from_le_bytes(bytes: &[u8]) -> kiln_error::Result<Self> {
// if bytes.len() < 16 {
// return Err(kiln_error::Error::runtime_execution_error("Insufficient bytes:
...
// kiln_error::Result<()> { writer.write_all(&self.bytes)
// }
// }

// Adding Error conversion for SerializationError -> kiln_error::Error
// This will be useful if functions returning kiln_error::Result need to
// propagate SerializationError.

impl From<SerializationError> for KilnError {
    fn from(e: SerializationError) -> Self {
        match e {
            SerializationError::IncorrectSize => KilnError::foundation_memory_provider_failed(
                "Incorrect buffer size for serialization/deserialization",
            ),
            SerializationError::InvalidFormat => KilnError::foundation_verification_failed(
            ...
            SerializationError::Custom(s) => {
                ...
                KilnError::foundation_verification_failed(s)
            },
            SerializationError::InvalidSliceLength => KilnError::foundation_memory_provider_failed(
                "Foundation invalid slice length for serialization",
            ),
            SerializationError::NotEnoughData => KilnError::foundation_memory_provider_failed(
                "Foundation not enough data to deserialize object",
            ),
            SerializationError::IoError => KilnError::foundation_memory_provider_failed(
                "Foundation I/O operation failed during serialization",
            ),
            SerializationError::UnexpectedEof => KilnError::foundation_memory_provider_failed(
                "Foundation unexpected end of input during deserialization",
            ),
            SerializationError::InvalidEnumValue => KilnError::foundation_verification_failed(
...
// Add necessary error codes to kiln_error::codes if they don't exist.
// For now, using generic SERIALIZATION_ERROR and DESERIALIZATION_ERROR.
// Need to check kiln-error/src/codes.rs for these.
...
// kiln-error/src/codes.rs". So these codes might already be there.
...
impl<'a> ReadStream<'a> {
    ...
    fn ensure_data(&self, len: usize) -> kiln_error::Result<()> {
        if self.position + len > self.buffer.len() {
            Err(KilnError::from(SerializationError::UnexpectedEof))
        } else {
            Ok(())
        }
    }

    pub fn read_u8(&mut self) -> kiln_error::Result<u8> {
        self.ensure_data(1)?;
    ...
    pub fn read_i8(&mut self) -> kiln_error::Result<i8> {
        self.read_u8().map(|v| v as i8)
    }

    // Helper for reading little-endian integers
    fn read_le_bytes_into_array<const N: usize>(&mut self) -> kiln_error::Result<[u8; N]> {
        self.ensure_data(N)?;
        let mut arr = [0u8; N];
    ...
    pub fn read_u16_le(&mut self) -> kiln_error::Result<u16> {
        self.read_le_bytes_into_array::<2>().map(u16::from_le_bytes)
    }

    pub fn read_i16_le(&mut self) -> kiln_error::Result<i16> {
        self.read_le_bytes_into_array::<2>().map(i16::from_le_bytes)
    }

    pub fn read_u32_le(&mut self) -> kiln_error::Result<u32> {
        self.read_le_bytes_into_array::<4>().map(u32::from_le_bytes)
    }

    pub fn read_i32_le(&mut self) -> kiln_error::Result<i32> {
        self.read_le_bytes_into_array::<4>().map(i32::from_le_bytes)
    }

    pub fn read_u64_le(&mut self) -> kiln_error::Result<u64> {
        self.read_le_bytes_into_array::<8>().map(u64::from_le_bytes)
    }

    pub fn read_i64_le(&mut self) -> kiln_error::Result<i64> {
        self.read_le_bytes_into_array::<8>().map(i64::from_le_bytes)
    }

    pub fn read_u128_le(&mut self) -> kiln_error::Result<u128> {
        self.read_le_bytes_into_array::<16>().map(u128::from_le_bytes)
    }

    pub fn read_i128_le(&mut self) -> kiln_error::Result<i128> {
        self.read_le_bytes_into_array::<16>().map(i128::from_le_bytes)
    }

    pub fn read_f32_le(&mut self) -> kiln_error::Result<f32> {
        self.read_le_bytes_into_array::<4>().map(f32::from_le_bytes)
    }

    pub fn read_f64_le(&mut self) -> kiln_error::Result<f64> {
        self.read_le_bytes_into_array::<8>().map(f64::from_le_bytes)
    }

    pub fn read_usize_le(&mut self) -> kiln_error::Result<usize> {
        if core::mem::size_of::<usize>() == 4 {
            ...
            Err(KilnError::system_error("Unsupported usize size for LE read"))
        }
    }

    pub fn read_isize_le(&mut self) -> kiln_error::Result<isize> {
        if core::mem::size_of::<isize>() == 4 {
            ...
            Err(KilnError::system_error("Unsupported isize size for LE read"))
        }
    }

    pub fn read_bool(&mut self) -> kiln_error::Result<bool> {
        match self.read_u8()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(KilnError::from(SerializationError::InvalidEnumValue)),
        }
    }

    /// Reads a slice of bytes from the stream.
    pub fn read_exact(&mut self, buf: &mut [u8]) -> kiln_error::Result<()> {
...
impl<'a> WriteStream<'a> {
    ...
    fn ensure_capacity(&self, len: usize) -> kiln_error::Result<()> {
        if self.position + len > self.buffer.len() {
            Err(KilnError::memory_error(
    ...
    pub fn write_u8(&mut self, value: u8) -> kiln_error::Result<()> {
    ...
    pub fn write_i8(&mut self, value: i8) -> kiln_error::Result<()> {
    ...
    pub fn write_u16_le(&mut self, value: u16) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_i16_le(&mut self, value: i16) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_u32_le(&mut self, value: u32) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_i32_le(&mut self, value: i32) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_u64_le(&mut self, value: u64) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_i64_le(&mut self, value: i64) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_u128_le(&mut self, value: u128) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_i128_le(&mut self, value: i128) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_f32_le(&mut self, value: f32) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_f64_le(&mut self, value: f64) -> kiln_error::Result<()> {
        self.write_le_bytes_from_array(value.to_le_bytes())
    }

    pub fn write_usize_le(&mut self, value: usize) -> kiln_error::Result<()> {
        if core::mem::size_of::<usize>() == 4 {
            self.write_u32_le(value as u32)
        } else if core::mem::size_of::<usize>() == 8 {
            self.write_u64_le(value as u64)
        } else {
            Err(KilnError::system_error(
                "Unsupported usize size for LE write",
            ))
        }
    }

    pub fn write_isize_le(&mut self, value: isize) -> kiln_error::Result<()> {
        if core::mem::size_of::<isize>() == 4 {
    ...
    pub fn write_bool(&mut self, value: bool) -> kiln_error::Result<()> {
        self.write_u8(if value { 1 } else { 0 })
    }

    /// Writes an entire slice of bytes into the stream.
    pub fn write_all(&mut self, bytes: &[u8]) -> kiln_error::Result<()> {
...
impl ToBytes for alloc::string::String {
    ...
    fn to_bytes_with_provider<'a, PStream: RootMemoryProvider>(
        ...
        writer.write_all(bytes).map_err(|_e| {
            KilnError::runtime_execution_error("Failed to write String data to stream")
        })
    }
}
...
impl FromBytes for alloc::string::String {
    fn from_bytes_with_provider<'a, PStream: crate::MemoryProvider>(
        ...
        reader.read_exact(&mut bytes).map_err(|_e| {
            KilnError::runtime_execution_error("Failed to read string bytes from stream")
        })?;

        // Convert to String
        alloc::string::String::from_utf8(bytes).map_err(|_e| {
            KilnError::runtime_execution_error("Invalid UTF-8 in string")
        })
    }
}
