//! Foundation Memory Safety Tests - MOVED
//!
//! The memory safety tests for kiln-foundation have been consolidated into
//! the main test suite at: kiln-tests/integration/memory/
//!
//! For the complete memory safety test suite, use:
//! ```
//! cargo test -p kiln-tests memory
//! ```
//!
//! Previously, foundation memory tests were in:
//! - kiln-foundation/tests/safe_memory_test.rs (MOVED)
//! - kiln-foundation/tests/safe_memory_tests.rs (MOVED)
//!
//! All functionality is now available in the consolidated test suite.

#[test]
fn foundation_memory_tests_moved_notice() {
    println!("Foundation memory safety tests have been moved to kiln-tests/integration/memory/");
    println!("Run: cargo test -p kiln-tests memory");
}
