//! Tests for fuel-bounded execution in kilnd
//!
//! This file contains tests that demonstrate the fuel-bounded execution
//! capabilities of the kilnd command-line tool.
...
mod tests {
    ...
    fn run_kilnd(wasm_file: &str, fuel: Option<u64>, call: Option<&str>) -> (bool, String) {
        // Find the kilnd binary in the target directory from the workspace root
        ...
        println!("Using kilnd at: {}", kilnd_path.display());
    ...
    fn test_fuel_bounded_execution() {
        ...
        let (success, output) = run_kilnd(&test_wasm, Some(10), Some("hello"));
        ...
        // We'll use the standard run_kilnd function but enhance the Command with --stats
        let project_root = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
            .parent()
            .unwrap()
            .to_path_buf();
